...
export async function savePlant(plant: PlantProps) : Promise<void> {
  try {
    ...
    //difference between now and nextTime in seconds
    ...
    const notificationId = await Notifications.scheduleNotificationAsync({
      ...
      trigger: {
        seconds: seconds < 60 ? 60 : seconds, //trigger minimum 60s before
        repeats: true
      }
    ...
    await AsyncStorage.setItem(
      '@plantmanager:plants', JSON.stringify({...newPlant, ...oldPlants})
    )
